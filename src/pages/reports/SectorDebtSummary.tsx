...
export function SectorDebtSummary() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.error('Error loading sector report:', error);
      toast.error(t('Failed to load sector debt summary'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const statusData = [
    ...
    { name: t('Rejected'), value: data.filter(r => r.status === 'rejected').length, color: '#ef4444' },
  ].filter(item => item.value > 0);

  const supervisorData = supervisors.map(s => ({
    name: s.full_name,
    debt: data.filter(r => r.supervisor_id === s.id).reduce((sum, r) => sum + r.total_amount, 0)
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-stone-900">{t('Sector Debt Summary')}</h1>
          <p className="text-stone-500">{t('Overview of debt for')} {sector?.name || t('your sector')}.</p>
        </div>
      ...
      {showFilters && (
        ...
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold text-stone-400 uppercase tracking-widest mb-1">{t('National ID')}</label>
              <input
                type="text"
                name="national_id"
            ...
            <div>
              <label className="block text-xs font-bold text-stone-400 uppercase tracking-widest mb-1">{t('Phone Number')}</label>
              <input
                type="text"
                name="phone"
                value={filters.phone}
            ...
            <div>
              <label className="block text-xs font-bold text-stone-400 uppercase tracking-widest mb-1">{t('Start Date')}</label>
              <input
                type="date"
                name="startDate"
            ...
            <div>
              <label className="block text-xs font-bold text-stone-400 uppercase tracking-widest mb-1">{t('End Date')}</label>
              <input
                type="date"
                name="endDate"
            ...
            <div>
              <label className="block text-xs font-bold text-stone-400 uppercase tracking-widest mb-1">{t('Supervisor')}</label>
              <select
                name="supervisor_id"
            ...
            <div>
              <label className="block text-xs font-bold text-stone-400 uppercase tracking-widest mb-1">{t('Worker Type')}</label>
              <select
                name="worker_type"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] uppercase font-bold text-stone-400 tracking-widest">{t('Total Sector Debt')}</span>
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] uppercase font-bold text-stone-400 tracking-widest">{t('Pending Sector Requests')}</span>
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] uppercase font-bold text-stone-400 tracking-widest">{t('Approved Sector Requests')}</span>
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] uppercase font-bold text-stone-400 tracking-widest">{t('Deducted Sector Debt')}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-stone-200 shadow-sm h-[400px]">
          <h3 className="text-sm font-bold text-stone-900 mb-6 uppercase tracking-widest">{t('Debt by Supervisor')}</h3>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={supervisorData}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f5f5f4" />
              <XAxis dataKey="name" fontSize={10} tickLine={false} axisLine={false} />
              <YAxis fontSize={10} tickLine={false} axisLine={false} tickFormatter={(value) => `RWF ${value/1000}k`} />
      ...
      <div className="bg-white rounded-xl border border-stone-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-stone-100 bg-stone-50 flex items-center justify-between">
          <h3 className="font-bold text-stone-900 uppercase tracking-widest text-xs">{t('Worker Debt List')}</h3>
